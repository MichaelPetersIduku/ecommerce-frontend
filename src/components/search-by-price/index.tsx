  ...
  const { isLoading, isError } = useQuery(['searchByPrice', min, max, page], () => {
    if(pageTitle !== PageTitle.SEARCH_BY_PRICE) {
      ...
      dispatch(actionCreator(ActionTypes.RESET_CURRENT_DISPLAY))
		...
		return fetch(`http://localhost:4321/api/v1/products/search?min=${min}&max=${max}&request=buyRequest`, {headers: {Authorization: "Basic anVkanNkc3Vfc29zOTpsZG85ZHNqbnM5OA=="}})
			.then((res) => res.json())
	}, 
  { 
