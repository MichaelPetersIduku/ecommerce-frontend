...
const SearchByP: FC<Prop> = ({ min, max }) => {
  const cs = useStyles();
  const {
    state: { currentDisplay, pageTitle, morePages, page },
    dispatch,
  } = useGlobalStore();
  const { isLoading, isError } = useQuery(
    ["searchByPrice", min, max, page],
    () => {
      if (pageTitle !== PageTitle.SEARCH_BY_PRICE) {
        dispatch(
          actionCreator(ActionTypes.PAGE_TITLE, PageTitle.SEARCH_BY_PRICE)
        );
        dispatch(actionCreator(ActionTypes.RESET_ALL_RES));
        dispatch(actionCreator(ActionTypes.RESET_CURRENT_DISPLAY));
      }
      return fetch(
        `https://api-ezewholesale.herokuapp.com/api/v1/products/search?min=${min}&max=${max}&request=buyRequest`,
        {
          headers: {
    ...
    },
    {
      ...
      onSuccess: (data: any) => {
        ...
        dispatch(
          actionCreator(ActionTypes.HOME_CURRENT_DISPLAY, isJson(data.data))
        );
        dispatch(actionCreator(ActionTypes.PAGINATOR, data.data));
  ...
  return (
    <div className={cs.root} ref={root}>
      {isLoading ? (
        <LoadingComponent
