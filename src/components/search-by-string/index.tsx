...
const SearchByStr: FC<Prop> = ({ query }) => {
  ...
  const { isLoading, isError } = useQuery(
    ["searchByString", query, page],
    () => {
      if (pageTitle !== PageTitle.SEARCH_BY_STRING) {
        dispatch(
          actionCreator(ActionTypes.PAGE_TITLE, PageTitle.SEARCH_BY_STRING)
        );
        dispatch(actionCreator(ActionTypes.RESET_ALL_RES));
        dispatch(actionCreator(ActionTypes.RESET_CURRENT_DISPLAY));
      }
      return fetch(
        `https://api-ezewholesale.herokuapp.com/api/v1/products/search?searchString=${query}&request=buyRequest`,
        {
          headers: {
