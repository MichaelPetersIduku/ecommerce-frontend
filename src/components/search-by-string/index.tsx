  ...
  const { isLoading, isError } = useQuery(['searchByString', query, page], () => {
    if(pageTitle !== PageTitle.SEARCH_BY_STRING) {
      ...
      dispatch(actionCreator(ActionTypes.RESET_CURRENT_DISPLAY));
		...
		return fetch(`http://localhost:4321/api/v1/products/search?searchString=${query}&request=buyRequest`, {headers: {Authorization: "Basic anVkanNkc3Vfc29zOTpsZG85ZHNqbnM5OA=="}})
			.then((res) => res.json())
	}, 
  { 
