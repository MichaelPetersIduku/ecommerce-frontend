...
const SideBarComponent: FC<SideBarProps> = ({modal}) => {
  ...
  const loadBuyRequest = async () => {
    try{
      ...
      if(modal) dispatch(actionCreator(ActionTypes.CLOSE));
      const data = await fetch(`http://localhost:4321/api/v1/googlesheet/runScript?scriptToRun=buyRequestScript`, {headers: {Authorization: "Basic anVkanNkc3Vfc29zOTpsZG85ZHNqbnM5OA=="}, method: "POST"});  
      console.log(data);
  ...
  const loadSellRequest = async () => {
    try{
      dispatch(actionCreator(ActionTypes.SHOW_MODAL));
      if(modal) dispatch(actionCreator(ActionTypes.CLOSE));
      const data = await fetch(`http://localhost:4321/api/v1/googlesheet/runScript?scriptToRun=sellRequestScript`, {headers: {Authorization: "Basic anVkanNkc3Vfc29zOTpsZG85ZHNqbnM5OA=="}, method: "POST"});  
      console.log(data);
      dispatch(actionCreator(ActionTypes.LOAD_IPHONE_ERROR, LoadIphoneEnum.SUCCESS))    
    } catch(e) {
      dispatch(actionCreator(ActionTypes.LOAD_IPHONE_ERROR, LoadIphoneEnum.FAILED))
