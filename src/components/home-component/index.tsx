...
const HomeComponent = () => {
  const cs = useStyles();
  const {
    state: { page, currentDisplay, pageTitle, morePages },
    dispatch,
  } = useGlobalStore();
    ...
    },
    {
      ...
      onSuccess: (data: any) => {
        ...
        if (data.status) {
          ...
          dispatch(
            actionCreator(ActionTypes.HOME_CURRENT_DISPLAY, data.data.docs)
          );
  ...
  return (
    <div className={cs.root} ref={root}>
      ...
      {isLoading ? (
        <LoadingComponent title="Loading ..." height="100%" width="100%" />
      ) : isError ? (
        <LoadingError />
      ) : (
