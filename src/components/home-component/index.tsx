...
const HomeComponent = () => {
  ...
  const { isLoading, isError } = useQuery(
    ...
    () => {
      ...
      return fetch(`http://localhost:4321/api/v1/products?request=buyRequest&page=${page+1}&limit=20`, {headers: {Authorization: "Basic anVkanNkc3Vfc29zOTpsZG85ZHNqbnM5OA=="}})
        .then((res) => res.json())
    }, 
    { 
      ...
      onSuccess: (data: any) => {
        data = isJson(data);
        if(data.status){
          dispatch(actionCreator(ActionTypes.HOME_ALL_RES, data.data.docs));
          dispatch(actionCreator(ActionTypes.HOME_CURRENT_DISPLAY, data.data.docs));
